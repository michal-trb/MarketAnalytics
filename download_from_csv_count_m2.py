...
class download_from_csv_count_m2:
    ...
    def add_csv(link_csv):
        ...
        df['date'] = df['quater'].apply(download_from_csv_count_m2.change_quater)
        df['year'] = df['year'].apply(str)
        df['date'] = df['year'].str.cat(df['date'])
        df['code'] = df['place'].str.replace('\s+', '').str.upper().str.replace('-', '') + df['date'].str.replace('-', '')
        df['code'] = df['code'].apply(download_from_csv_count_m2.remove_polish_chars)
    ...
    def upload_data(con, cursor, df):
        ...
        for index, row in df.iterrows():
            cursor.execute("INSERT INTO polish_market_count_m2 (code, place, quater, transactions, area, year, value, unit, date) values(?,?,?,?,?,?,?,?,?)",
...
if __name__ == '__main__':
    ...
    df_polish_market = download_from_csv_count_m2.add_csv('csv_files/suma_m2.csv')
    con, cursor = cs.sql_connect()
    download_from_csv_count_m2.upload_data(con, cursor, df_polish_market)
